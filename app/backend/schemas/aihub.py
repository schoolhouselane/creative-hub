...
class GenTxtRequest(BaseModel):
    ...
    model: str = Field(default="gemini-3.5-flash", description="Model name")
...
class ConvMessage(BaseModel):
    """A single turn in a multi-turn image generation conversation."""
    role: str = Field(..., description="'user' or 'assistant'")
    content: str = Field(default="", description="Text content of the turn.")
    image_url: Optional[str] = Field(default=None, description="AI-generated image as data URI (assistant turns).")
    attached_image: Optional[str] = Field(default=None, description="User-uploaded image as data URI (user turns).")
...
class GenImgRequest(BaseModel):
    ...
    messages: Optional[List[ConvMessage]] = Field(
        default=None,
        description="Optional conversation history for multi-turn image generation (Gemini chat mode).",
    )
    brand_context: Optional[str] = Field(
        default=None,
        description="Full brand guidelines brief injected as Gemini system instruction.",
    )
    product_images: Optional[List[str]] = Field(
        default=None,
        description="Product reference images (base64 data URIs) appended to the current user turn so Gemini can faithfully reproduce specific brand products.",
    )
    model: str = Field(
        default="gemini-3-pro-image",
        description="Model name",
    )
    size: str = Field(default="1:1", description="Image aspect ratio: 1:1 / 9:16 / 16:9.")
