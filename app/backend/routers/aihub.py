...
class FluxGenImgRequest(BaseModel):
    prompt: str
    size: str = "square_hd"
    model: str = "flux-pro"


class FluxGenImgResponse(BaseModel):
    images: list[str]
    model: str = "flux-pro"


@router.post("/genimg-flux", response_model=FluxGenImgResponse)
async def generate_image_flux(request: FluxGenImgRequest):
    """Generate brand-quality images via fal.ai Flux — best quality for design assets."""
    try:
        service = AIHubService()
        images = await service.genimg_flux(request.prompt, request.size, request.model)
        return FluxGenImgResponse(images=images, model=request.model)
    except ValueError as e:
        logger.error(f"Flux config error: {e}")
...
# Keep old endpoint name as alias so existing frontend calls still work
@router.post("/genimg-freepik", response_model=FluxGenImgResponse)
async def generate_image_flux_alias(request: FluxGenImgRequest):
    """Alias for /genimg-flux — redirects to Flux generation."""
    return await generate_image_flux(request)


class FluxPulidRequest(BaseModel):
    prompt: str
    reference_image: str
    size: str = "square_hd"


class FluxPulidResponse(BaseModel):
    images: list[str]
    model: str = "flux-pulid"


@router.post("/genimg-pulid", response_model=FluxPulidResponse)
async def generate_image_pulid(request: FluxPulidRequest):
    """Generate images with a consistent face/avatar via fal.ai Flux PuLID.

    Upload a reference photo once; all generated images will preserve that face
    across different scenes, styles, and brand contexts.
    """
    try:
        service = AIHubService()
        images = await service.genimg_flux_pulid(request.prompt, request.reference_image, request.size)
        return FluxPulidResponse(images=images)
    except ValueError as e:
        logger.error(f"PuLID config error: {e}")
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=str(e))
    except Exception as e:
        logger.error(f"PuLID generation failed: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=extract_error_message(e))
...
@router.post("/extract-brand-dna")
async def extract_brand_dna_from_pdf(file: UploadFile = File(...)):
    """
    Extract structured brand DNA from a uploaded PDF using PyMuPDF + Gemini.
    Returns the raw DNA dict — no DB writes, no brand folder.
    Used by the QuickUploadZone before a brand record exists.
    """
    from services.brand_guidelines import extract_text_from_pdf, extract_brand_dna, extract_logo_from_pdf
    ...
    try:
        ...
        if not pdf_text.strip():
            raise HTTPException(
                status_code=422,
                detail="Could not extract text from PDF — it may be image-only or scanned. Try a text-based PDF."
            )

        service = AIHubService()
        # Run text extraction and logo extraction concurrently
        import asyncio as _asyncio
        dna, logo_url = await _asyncio.gather(
            extract_brand_dna(pdf_text, service),
            _asyncio.get_event_loop().run_in_executor(None, extract_logo_from_pdf, pdf_bytes),
        )

        if not dna.get("brand_name") and not dna.get("parse_error"):
            raise HTTPException(status_code=422, detail="AI could not identify a brand name in this document")

        return {"dna": dna, "logo_url": logo_url, "text_length": len(pdf_text), "success": True}

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"extract-brand-dna failed: {e}", exc_info=True)
